	...
	public class AccessTokenHandler : AuthenticationHandler<AccessTokenAuthenticationOptions>
	{
		protected override Task<AuthenticateResult> HandleAuthenticateAsync()
		{
			try
			{
				...
				if(token.Count == 0)
                {
					token = Context.Request.Headers[AccessTokenDefaults.AccessTokenQueryName];
				}
				if (Context.Request.Path.Value == "/wopibootstrapper")
                {
                    ...
                    token = Options.SecurityHandler.WriteToken(Options.SecurityHandler.GenerateAccessToken(0, "Anonymous", "Anonymous"));
